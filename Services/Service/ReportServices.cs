...
namespace Services.Service
{
    public class ReportServices : IReport
    {
        ...
        public async Task<Result<ReportSummaryDTO>> Summary(ClaimsPrincipal claims, DateTime? start = null, DateTime? end = null)
        {
            ...
            (
                i.User_Id == ID &&
                (!start.HasValue || i.Date_Deposite >= start) &&
                (!end.HasValue || i.Date_Deposite <= end)
            ));
            IEnumerable<IncomeDTO> Incomes = (await _cache.GetAllfromCache<IncomeDTO>(ID));
            if (Incomes is null)
            {
                Incomes = unitOfWork.Repository<Income>().FindAll(specIncome).Select(
                    i => new IncomeDTO
                    {
                        Id = i.Id,
                        Amount = i.Amount,
            ...
            var report = new ReportSummaryDTO()
            {
                ...
                TopIncomeSource = string.Join(".\n", topIncome),
                TopExpenseCategory = string.Join(".\n", topExpenses),
                BudgetStatus = status
            };
